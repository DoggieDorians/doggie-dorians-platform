...
|--------------------------------------------------------------------------
| PURPOSE
| - Worker-safe endpoint/page to claim an open job
| - Only allows claiming open / unassigned jobs
| - Prevents workers from taking jobs already assigned to someone else
|--------------------------------------------------------------------------
*/
...
if (!in_array($currentRole, $allowedWorkerRoles, true)) {
    $_SESSION['walker_flash_type'] = 'error';
    $_SESSION['walker_flash_message'] = 'You do not have permission to accept jobs.';
...
if ($jobId <= 0) {
    ...
    header('Location: walker-available.php');
    exit;
}
...
if (!tableExists($pdo, $BOOKINGS_TABLE)) {
    $error = "The bookings table was not found. Update \$BOOKINGS_TABLE in walker-accept-job.php if needed.";
} else {
    try {
        ...
        if ($walkerIdCol === null) {
            ...
            if (!$job) {
                ...
                if ($assignedWorkerId > 0 && $assignedWorkerId !== $workerId) {
                    $error = 'This job has already been assigned to another worker.';
                } elseif ($assignedWorkerId === $workerId) {
                    $error = 'This job is already assigned to you.';
                } elseif (!in_array($statusRaw, $openStatuses, true)) {
                    $error = 'This job is not currently available to claim.';
                    ...
                    if (!$currentJob) {
                        ...
                        if ($currentAssignedWorkerId > 0 && $currentAssignedWorkerId !== $workerId) {
                            $pdo->rollBack();
                            $error = 'Another worker claimed this job first.';
                        } elseif (!in_array($currentStatusRaw, $openStatuses, true)) {
                            $pdo->rollBack();
                            $error = 'This job is no longer open for claiming.';
                        } else {
                            $sqlUpdate = "
                                UPDATE $BOOKINGS_TABLE
                                SET
                                    $walkerIdCol = :worker_id,
                            ...
                            $job['status_name'] = $assignedStatus;
                            $accepted = true;
                            $success = 'Job successfully accepted and added to your assigned jobs.';

                            $_SESSION['walker_flash_type'] = 'success';
                            $_SESSION['walker_flash_message'] = 'Job accepted successfully.';
                        }
                    }
                }
            }
        ...
        $error = 'Accept job error: ' . $e->getMessage();
    }
}
...
<head>
    ...
    <title>Accept Job | Doggie Dorian’s</title>
    <meta name="description" content="Worker-safe job acceptance page for Doggie Dorian’s.">
    <style>
    ...
    <div class="container">
        ...
        <div class="topbar">
            <div>
                ...
                <h1 class="headline">Accept Job</h1>
                <p class="subheadline">
                    Review this open service before adding it to your own assigned workload.
                </p>
            </div>
        ...
        <?php if ($job !== null): ?>
            ...
            $statusLabel = niceStatus((string)($job['status_name'] ?? 'Open'));
            $scheduledLabel = formatJobDate((string)($job['date_value'] ?? ''), (string)($job['time_value'] ?? ''));
            ?>
            <section class="hero">
                <div class="hero-top">
                    <div>
                        <div class="eyebrow"><?= $accepted ? 'Job Claimed' : 'Open Opportunity' ?></div>
                    ...
                    <div class="status-pill"><?= h($accepted ? niceStatus($assignedStatus) : $statusLabel) ?></div>
                </div>

                <div class="hero-actions">
                    <?php if (!$accepted && $error === ''): ?>
                        <form method="post" action="walker-accept-job.php?id=<?= urlencode((string)$job['id']) ?>" style="display:inline;">
                            <button type="submit" class="btn" onclick="return confirm('Accept this job into your workload?');">
                                Accept Job
                            </button>
                        </form>
                    <?php endif; ?>
                    ...
                    <?php if ($accepted): ?>
                        <a class="btn" href="walker-jobs.php">Go To My Jobs</a>
                        <a class="btn-secondary" href="walker-job-view.php?id=<?= urlencode((string)$job['id']) ?>">Open Job Details</a>
                    <?php else: ?>
                        <a class="btn-secondary" href="walker-job-view.php?id=<?= urlencode((string)$job['id']) ?>">View Job Page</a>
                    <?php endif; ?>

                    <a class="btn-secondary" href="walker-available.php">Back To Available Jobs</a>
                </div>
            </section>
            ...
            <div class="grid">
                <section class="panel">
                    <div class="panel-header">
                        <div>
                            ...
                            <p class="panel-subtitle">Review the booking before claiming it</p>
                        </div>
                    ...
                    <div class="details-grid">
                        ...
                        <div class="detail-card">
                            ...
                            <div class="detail-value"><?= h($accepted ? niceStatus($assignedStatus) : $statusLabel) ?></div>
                        </div>
                ...
                <section class="panel">
                    <div class="panel-header">
                        <div>
                            <h2 class="panel-title">Acceptance flow</h2>
                            <p class="panel-subtitle">What happens when you claim this job</p>
                        </div>
                    </div>

                    <div class="action-stack">
                        <div class="action-card">
                            <h3 class="action-title">Assignment updates</h3>
                            <p class="action-copy">
                                Accepting this job assigns it directly to your worker account and removes it from the open queue.
                            </p>
                        </div>

                        <div class="action-card">
                            <h3 class="action-title">Status update</h3>
                            <p class="action-copy">
                                The booking status is moved into an assigned state so it can appear in your worker dashboard and ready-to-start list.
                            </p>
                        </div>
                        ...
                        <div class="action-card">
                            ...
                            <p class="action-copy">
                                After acceptance, open the job from your assigned jobs list and start the service when you are ready.
                            </p>

                            <?php if (!$accepted && $error === ''): ?>
                                <form method="post" action="walker-accept-job.php?id=<?= urlencode((string)$job['id']) ?>">
                                    <button type="submit" class="mini-btn" onclick="return confirm('Accept this job into your workload?');">
                                        Confirm Acceptance
                                    </button>
                                </form>
                            <?php else: ?>
                                <a class="mini-btn-ghost" href="walker-jobs.php">Open My Jobs</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
        ...
        <div class="footer-note">
            Signed in as <?= h($workerEmail !== '' ? $workerEmail : $displayName) ?> · Worker-safe acceptance flow enforced
        </div>
    </div>
</body>
</html>