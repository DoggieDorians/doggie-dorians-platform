...
try {
    if (!tableExists($pdo, $USERS_TABLE)) {
        $error = "The users table was not found. Update \$USERS_TABLE in admin-unassign-walker.php if needed.";
    } elseif (!tableExists($pdo, $BOOKINGS_TABLE)) {
        ...
        if (!in_array($USER_ID_COL, $userColumns, true)) {
        ...
        } elseif (!in_array($BOOKING_ID_COL, $bookingColumns, true)) {
            $error = 'Booking ID column not found in bookings table.';
        } else {
            $schema['user_name_col'] = firstExistingColumn($possibleUserNameCols, $userColumns);
            $schema['user_email_col'] = firstExistingColumn($possibleUserEmailCols, $userColumns);
            $schema['user_role_col'] = firstExistingColumn($possibleUserRoleCols, $userColumns);
            ...
            if ($schema['booking_walker_col'] === null) {
                $error = 'No worker assignment column was found in bookings. Add walker_id, staff_id, or employee_id.';
            } elseif ($schema['booking_status_col'] === null) {
                $error = 'No booking status column was found in bookings. Add status or booking_status.';
            }
        }
    }
} catch (Throwable $e) {
...
if ($error === '') {
    try {
        $jobSelectParts = [
            $BOOKING_ID_COL . ' AS booking_id',
            $schema['booking_walker_col'] . ' AS assigned_worker_id',
            $schema['booking_status_col'] . ' AS status_name'
        ];

        $jobSelectParts[] = $schema['booking_service_col'] ? $schema['booking_service_col'] . ' AS service_name' : "'' AS service_name";
        $jobSelectParts[] = $schema['booking_date_col'] ? $schema['booking_date_col'] . ' AS date_value' : "'' AS date_value";
        $jobSelectParts[] = $schema['booking_time_col'] ? $schema['booking_time_col'] . ' AS time_value' : "'' AS time_value";
        $jobSelectParts[] = $schema['booking_created_col'] ? $schema['booking_created_col'] . ' AS created_value' : "'' AS created_value";
        $jobSelectParts[] = $schema['booking_pet_col'] ? $schema['booking_pet_col'] . ' AS pet_name_value' : "'' AS pet_name_value";
        $jobSelectParts[] = $schema['booking_notes_col'] ? $schema['booking_notes_col'] . ' AS notes_value' : "'' AS notes_value";
        $jobSelectParts[] = $schema['booking_address_col'] ? $schema['booking_address_col'] . ' AS address_value' : "'' AS address_value";
        $jobSelectParts[] = $schema['booking_client_col'] ? $schema['booking_client_col'] . ' AS client_value' : "'' AS client_value";

        $sqlJob = "
            SELECT
                " . implode(",\n                ", $jobSelectParts) . "
            FROM $BOOKINGS_TABLE
            WHERE $BOOKING_ID_COL = :job_id
            LIMIT 1
        ";

        $stmtJob = $pdo->prepare($sqlJob);
        $stmtJob->execute([':job_id' => $jobId]);
        $job = $stmtJob->fetch(PDO::FETCH_ASSOC) ?: null;
        ...
        if (!$job) {
            ...
            if ($assignedWorkerId > 0) {
                ...
                $workerSelectParts[] = $schema['user_name_col'] ? $schema['user_name_col'] . ' AS worker_name' : "'' AS worker_name";
                $workerSelectParts[] = $schema['user_email_col'] ? $schema['user_email_col'] . ' AS worker_email' : "'' AS worker_email";
                $workerSelectParts[] = $schema['user_role_col'] ? $schema['user_role_col'] . ' AS worker_role' : "'' AS worker_role";
                ...
                $stmtWorker->execute([':worker_id' => $assignedWorkerId]);
                $assignedWorker = $stmtWorker->fetch(PDO::FETCH_ASSOC) ?: null;
            }
        }
    } catch (Throwable $e) {
    ...
    try {
        ...
        if (!$currentJob) {
            ...
            $sqlUpdate = "
                ...
                SET
                    {$schema['booking_walker_col']} = NULL,
                    {$schema['booking_status_col']} = :booking_status
                WHERE $BOOKING_ID_COL = :job_id
            ";
