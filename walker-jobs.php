...
|--------------------------------------------------------------------------
| PURPOSE
| - Worker-safe jobs listing page
| - Shows only the logged-in worker's assigned jobs
| - Splits jobs into ready / live / completed sections
|--------------------------------------------------------------------------
*/
...
if (!in_array($currentRole, $allowedWorkerRoles, true)) {
    $_SESSION['walker_flash_type'] = 'error';
    $_SESSION['walker_flash_message'] = 'You do not have permission to access worker jobs.';
...
if (!tableExists($pdo, $BOOKINGS_TABLE)) {
    $error = "The bookings table was not found. Update \$BOOKINGS_TABLE in walker-jobs.php if needed.";
} else {
    try {
        ...
        $error = 'Jobs page error: ' . $e->getMessage();
    }
}
...
<head>
    ...
    <title>My Jobs | Doggie Dorian’s</title>
    <meta name="description" content="Worker jobs page for Doggie Dorian’s.">
    <style>
    ...
    <div class="container">
        ...
        <div class="topbar">
            <div>
                ...
                <h1 class="headline"><?= h($firstName) ?>’s Jobs</h1>
                <p class="subheadline">
                    This page shows only the jobs assigned to your worker account, grouped by what needs attention now.
                </p>
            </div>
        ...
        <section class="hero">
            <div class="eyebrow">Worker Job Overview</div>
            <h2 style="margin:0; font-size:28px; letter-spacing:-0.03em;">Your assigned workload</h2>
            <p class="subheadline" style="margin-top:10px;">
                Start upcoming services, continue active work, and review completed history from one worker-safe page.
            </p>

            <div class="hero-grid">
                <div class="stat">
                    <div class="stat-label">All Jobs</div>
                    <div class="stat-value"><?= h((string)$stats['all_jobs']) ?></div>
                    <div class="stat-note">All jobs currently tied to your account</div>
                </div>

                <div class="stat">
                    <div class="stat-label">Ready To Start</div>
                    <div class="stat-value"><?= h((string)$stats['ready']) ?></div>
                    <div class="stat-note">Assigned services waiting for you to begin</div>
                </div>

                <div class="stat">
                    <div class="stat-label">In Progress</div>
                    <div class="stat-value"><?= h((string)$stats['live']) ?></div>
                    <div class="stat-note">Live services currently underway</div>
                </div>

                <div class="stat">
                    <div class="stat-label">Completed</div>
                    <div class="stat-value"><?= h((string)$stats['completed']) ?></div>
                    <div class="stat-note">Finished jobs in your visible history</div>
                </div>
            </div>
        </section>
        ...
        <div class="stack">
            <section class="panel">
                <div class="panel-header">
                    <div>
                        ...
                        <p class="panel-subtitle">Services assigned to you and ready for the worker start flow</p>
                    </div>
                    <div class="badge"><?= h((string)$stats['ready']) ?> Ready</div>
                </div>

                <?php if (empty($readyJobs)): ?>
                    <div class="empty-state">
                        No assigned jobs are waiting to be started right now.
                    </div>
                <?php else: ?>
                    <div class="job-list">
            ...
            <section class="panel">
                <div class="panel-header">
                    <div>
                        ...
                        <p class="panel-subtitle">Live jobs already started and currently in the worker tracking flow</p>
                    </div>
                    <div class="badge"><?= h((string)$stats['live']) ?> Live</div>
                </div>
                            ...
                            <article class="job-card">
                                ...
                                <div class="job-grid">
                                    ...
                                    <div class="job-item">
                                        ...
                                        <strong><?= h(formatDateTimeValue((string)($job['started_at_value'] ?? ''), 'Not stored')) ?></strong>
                                    </div>
                                    <div class="job-item">
                                        <span>Elapsed</span>
            ...
            <section class="panel">
                <div class="panel-header">
                    <div>
                        <h2 class="panel-title">Completed jobs</h2>
                        <p class="panel-subtitle">Finished services preserved in your worker history</p>
                    </div>
                    <div class="badge"><?= h((string)$stats['completed']) ?> Completed</div>
                </div>

                <?php if (empty($completedJobs)): ?>
                    <div class="empty-state">
                        No completed jobs were found yet.
                    </div>
                <?php else: ?>
                    <div class="job-list">
                        <?php foreach ($completedJobs as $job): ?>
                            <?php $statusLabel = niceStatus((string)($job['status_name'] ?? 'Completed')); ?>
                            <article class="job-card">
                                <div class="job-top">
                                    <div>
                                        <h3 class="job-title"><?= h(niceService((string)($job['service_name'] ?? 'Service'))) ?> #<?= h((string)$job['id']) ?></h3>
                                    ...
                                    <div class="status-pill status-completed"><?= h($statusLabel) ?></div>
                                </div>
                                ...
                                <div class="job-grid">
                                    ...
                                    <div class="job-item">
                                        ...
                                        <strong><?= h(formatDateTimeValue((string)($job['completed_at_value'] ?? ''), 'Not stored')) ?></strong>
                                    </div>
                                    <div class="job-item">
                                        <span>Duration</span>
        ...
        <div class="footer-note">
            Signed in as <?= h($workerEmail !== '' ? $workerEmail : $displayName) ?> · Worker-only job visibility enforced
        </div>
    </div>
</body>
</html>