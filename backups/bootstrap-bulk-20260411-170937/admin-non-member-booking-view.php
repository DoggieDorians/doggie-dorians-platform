...
function service_badge_class(string $serviceType): string
{
    return match ($serviceType) {
        ...
        'Boarding' => 'boarding',
        'Drop-In' => 'dropin',
        'Drop-In + Walk' => 'dropinwalk',
        'Sitting', 'In-Home Sitting' => 'sitting',
        default => 'default',
    };
}
...
|--------------------------------------------------------------------------
*/
$flashType = $_SESSION['admin_nonmember_flash_type'] ?? '';
$flashMessage = $_SESSION['admin_nonmember_flash_message'] ?? '';

unset(
    $_SESSION['admin_nonmember_flash_type'],
    $_SESSION['admin_nonmember_flash_message']
);
...
if ($serviceType === 'Walk') {
    ...
    if (!empty($booking['preferred_walk_time'])) {
        $parts[] = 'Preferred time: ' . $booking['preferred_walk_time'];
    }
    $serviceSummary = !empty($parts) ? implode(' • ', $parts) : '—';
} elseif ($serviceType === 'Daycare') {
    $serviceSummary = 'Daycare request';
    if (!empty($booking['dog_size'])) {
        $serviceSummary .= ' • ' . $booking['dog_size'] . ' dog';
    }
} elseif ($serviceType === 'Boarding') {
    $serviceSummary = 'Boarding request';
    if (!empty($booking['dog_size'])) {
        $serviceSummary .= ' • ' . $booking['dog_size'] . ' dog';
    }
    ...
    if (!empty($booking['dropin_preferred_time'])) {
        $parts[] = 'Preferred start: ' . $booking['dropin_preferred_time'];
    }
    $serviceSummary = !empty($parts) ? implode(' • ', $parts) : '—';
} elseif ($serviceType === 'Drop-In + Walk') {
    $parts = [];
    if (!empty($booking['dropin_hours'])) {
        $parts[] = (int) $booking['dropin_hours'] . ' hour visit';
    }
    ...
    if (!empty($booking['dropin_walk_preferred_time'])) {
        $parts[] = 'Preferred start: ' . $booking['dropin_walk_preferred_time'];
    }
    $serviceSummary = !empty($parts) ? implode(' • ', $parts) : '—';
} elseif ($serviceType === 'Sitting' || $serviceType === 'In-Home Sitting') {
    $parts = [];
    $parts[] = 'In-home sitting session';

    if (!empty($booking['date_start'])) {
        $parts[] = 'Start: ' . format_date($booking['date_start']);
    }

    if (!empty($booking['date_end'])) {
        $parts[] = 'End: ' . format_date($booking['date_end']);
    }
    ...
    <main class="content">
      ...
      <section class="top-grid">
        <div class="panel">
          ...
          <div class="summary-card">
            <div class="summary-row">
              ...
              <div class="summary-value"><?php echo h($booking['booking_reference'] ?? 'N/A'); ?></div>
            </div>
        ...
        <div class="panel">
          ...
          <form class="status-form" action="process-admin-non-member-booking-update.php" method="post">
      ...
      <section class="panel">
        ...
        <div class="panel-head-copy">
          The service selection and all submitted timing details for this non-member request.
        </div>
      ...
      <section class="panel">
        ...
        <div class="notes-box"><?php echo trim((string) ($booking['notes'] ?? '')) !== '' ? h($booking['notes']) : 'No additional notes were submitted.'; ?></div>
      </section>
              ...
              <?php foreach ($rawFormData as $key => $value): ?>
                <div class="raw-item">
                  ...
                  <div class="raw-value">
                    <?php
                    if (is_array($value)) {
                        echo h(json_encode($value, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
                    } else {
                        echo h((string) $value);
                    }
                    ?>
