...
function buildUsernameFromName(string $firstName, string $lastName, string $email): string
{
    ...
    if ($seed === '') {
        $emailLeft = strtolower(trim((string)strtok($email, '@')));
        $emailLeft = preg_replace('/[^a-z0-9]/', '', $emailLeft ?? '');
        $seed = $emailLeft ?? '';
    }
        ...
        try {
            ...
            if ($existingUser) {
                ...
                if (tableExists($pdo, 'members')) {
                    ...
                    if (hasColumn($memberColumns, 'username')) {
                        $memberData['username'] = buildUsernameFromName($form['first_name'], $form['last_name'], $form['email']);
                    }
                    ...
                    if (hasColumn($memberColumns, 'email_verified')) {
                        $memberData['email_verified'] = 1;
                    }

                    if (hasColumn($memberColumns, 'verification_token')) {
                        $memberData['verification_token'] = null;
                    }
